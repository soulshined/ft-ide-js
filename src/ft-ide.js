...
DOMElement.prototype.createTextNode = function(text) {
  this.node = document.createTextNode(text);
  return this;
}
DOMElement.prototype.cloneElement = function(element, includeChildren = true) {
  this.node = (element instanceof DOMElement) 
    ? element.node.cloneNode(includeChildren) 
    : element.cloneNode(includeChildren);
  return this;
}
DOMElement.prototype.innerHTML = function(html) {
  this.node.innerHTML = html;
  return this;
}
DOMElement.prototype.innerText = function(text) {
  this.node.innerText = text;
  return this;
}
DOMElement.prototype.styles = function(styles) {
...
DOMElement.prototype.title = function(title) {
  this.node.title = title;
  return this;
}
DOMElement.prototype.addClass = function() {
  this.node.className += ` ${[...arguments].join(' ')}`;
  return this;
}
...
DOMElement.prototype.appendChild = function(child) {
  this.node.appendChild((child instanceof DOMElement) ? child.node : child);
  return this;
}
DOMElement.prototype.appendChildren = function() {
  [...arguments].forEach(element => {
    this.node.appendChild((element instanceof DOMElement) ? element.node : element);
  });
  return this;
}
const FTIDETreeHandler = function() {
}

FTIDETreeHandler.prototype.generateTree = function(tree, options) {
  let content = [];
  let cIndex = 0;
  let cDepth = 0;

  FTIDETreeHandler.prototype._prepTree = function (tree) {
    const nav = new DOMElement()
      .createElement('NAV')
      .addClass('explorer', options.explorerStyle)
      .appendChild(this._generateTree(tree).addClass('tree'));

    return nav;
  }

  FTIDETreeHandler.prototype._generateLI = function(content, isFolder = false) {
    const li = new DOMElement().createElement('LI')
      .addClass(isFolder ? 'folder' : 'item')
      .styles(`padding-left: ${cDepth*15}px;`)
      .appendChild(new DOMElement().createTextNode(content));

    if (options.expandFolders && isFolder) li.addClass('open');
    if (!isFolder) li.setAttribute('data-ft-ide-id', cIndex++);

    return li;
  };

  FTIDETreeHandler.prototype._generateTree = function(tree) {
    const ul = new DOMElement().createElement('UL');

    for (const [key, value] of Object.entries(tree)) {
      if (key.trim().length === 0) continue;
      
      if (typeof value === 'object') {
        ul.appendChild(this._generateLI(key, true));
        cDepth++;
        let child = this._generateTree(value);
        if (child.node.firstChild) ul.appendChild(child);
      }
      else {
        ul.appendChild(this._generateLI(key));
        content.push(value);
      }
    }
    cDepth--;
  ...
  return new Promise((resolve, reject) => {
    if (typeof tree !== 'object')
      reject('1st Argument expecting an object of kvps');

    resolve({html: this._prepTree(tree).node, content: content});
  })
}

/* OPTIONS */
const FTIDEOptions = function(options) {  
  const format = function (val) {
    return val.toString().trim().toLowerCase();
  }
  const defineOption = function(val, defaultOption, allowedOptions = [], isStrict = false) {
    if (val === undefined || val === null) 
      return defaultOption;

    if (allowedOptions.length > 0) {
      if (!allowedOptions.includes(format(val))) {
        if (options.isDebug)
          console.warn(`[FTIDEOptions] invalid property value; expecting one of these values: ${allowedOptions.join()}`);
        return defaultOption;
      }
    }

    return isStrict === true ? val : format(val);
  }
  ...
  return {
    ...
    expandFolders : options.expandFolders || false,
    explorerStyle: defineOption(options.explorerStyle, 'material', ['material','compact']),
    ideFontFamily: options.ideFontFamily || null,
    isDebug: options.isDebug || false,
    maxBreadcrumbs: options.maxBreadcrumbs || 4,
    rootPath: options.rootPath || null,
    showBreadcrumbs: defineOption(options.showBreadcrumbs, 'oncollapse', ['always','oncollapse','never']),
    showCopyToClipboard : options.showCopyToClipboard === false ? false : true,
    syntaxHighlighter : {
      highlight: options.syntaxHighlighter.highlight === false ? false : true,
      class : defineOption(options.syntaxHighlighter.class, 'prettyprint', [], true)
    },
    theme : defineOption(options.theme, 'default', ['default', 'dark'])
  }
}

/* MAIN */
const FTIDE = function(element_id, tree, options = {}) {
  this.ide = document.getElementById(element_id);
  if (this.ide === null)
  ...
  if (this.options.isDebug) {
    console.log(`FTIDEOptions for #${element_id}:`);
    console.table(this.options);
  }
  this.ide.setAttribute('data-ft-ide-theme', this.options.theme);

  new FTIDETreeHandler()
    .generateTree(tree, this.options)
    .then((tree) => {
      this._writeIDEToDOM(tree);
      this._writeStylesToIDE();
      this._registerLIEvents();
    })
    .catch(err => {
      throw err;
    })

  FTIDE.prototype._writeIDEToDOM = function (tree) {
    this.ide.innerHTML = `<!--
$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
$$$    FT-IDE CREATED BY github.com/soulshined    $$$
$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
-->`;
    ...
    const mobileNav = new DOMElement().cloneElement(tree.html)
      .addClass('mobile');      
    this.mobileNav = mobileNav.node;
    this.ide.appendChild(mobileNav.node);

    //prep the content-wrapper
    const contentWrapper = new DOMElement().createElement('DIV').addClass('content-wrapper');

    if (this.options.showBreadcrumbs !== 'never') {
      const breadcrumbDiv = new DOMElement().createElement('UL').addClass('ft-ide-breadcrumb');

      if (this.options.showBreadcrumbs === 'always') 
        breadcrumbDiv.styles('display: flex !important');

      this.breadcrumbDiv = breadcrumbDiv;
      contentWrapper.appendChild(breadcrumbDiv);
    }

    const contentDiv = new DOMElement()
      .createElement('CODE')
      .addClass('content')
      .styles('line-height: 1.4; user-select: none !important;');

    const preDiv = new DOMElement()
      .createElement('PRE')
      .appendChild(contentDiv);
      
    if (this.options.syntaxHighlighter.highlight) 
      contentDiv.addClass(this.options.syntaxHighlighter.class);
    
    contentWrapper.appendChild(preDiv);
    this.contentDiv = contentDiv.node;    
    
    //prep mobile friendly explorer
    const mobileactions = new DOMElement().createElement('DIV').addClass('mobile-actions');
    
    const maExplorerBtn = new DOMElement().createElement('BUTTON')
      ...
      .title('Explorer');
    maExplorerBtn.node.onclick = this._toggleMobileExplorer.bind(this);

    mobileactions.appendChild(maExplorerBtn);

    if (this.options.showCopyToClipboard) {
      const maCopyBtn = new DOMElement().createElement('BUTTON')
      ...
      maCopyBtn.node.onclick = this._handleCopyToClipboard.bind(this);

      let copyBtn = new DOMElement().cloneElement(maCopyBtn).id('ftide__btn-copy');
      copyBtn.node.onclick = this._handleCopyToClipboard.bind(this);

      this.ide.appendChild(copyBtn.node);
      mobileactions.appendChild(maCopyBtn);
    }
    
    //append all elements to IDE
    this.ide.appendChild(tree.html);
    this.ide.appendChild(mobileactions.node);
    this.ide.appendChild(contentWrapper.node);
    ...
    if (this.content.length > 0) {
      ...
      this._setContent(this.content[contentid]);
    }
  }
  FTIDE.prototype._toggleMobileExplorer = function() {
      ...
      nav.addEventListener('click', (e) => {
        ...
        if (target && target.nodeName === 'LI') {
          ...
          if (!target.classList.contains('folder')) {
            ...
            this._setContent(this.content[target.getAttribute('data-ft-ide-id')]);
            ...
            //make LI active item
            [...document.querySelectorAll(`#${this.ide.id} nav.explorer li`)].map(m => m.classList.remove('active'));
          ...
          if (options.showBreadcrumbs !== 'never') {
            let lis = [];
            let node = target.parentNode;
            while (!node.classList.contains('tree')) {
              lis.push(new DOMElement().createElement('LI').innerText(node.previousSibling.innerText));
              node = node.parentNode;
            }
            if (lis.length >= this.options.maxBreadcrumbs) {
              lis = lis.slice(-Math.abs(this.options.maxBreadcrumbs));
              lis.unshift(new DOMElement().createElement('LI').innerText('...'));
            }
            this.breadcrumbDiv.innerHTML([...lis].reverse().map(m => m.node.outerHTML).join(''));
          }
        }
      })      
    });
  }
  FTIDE.prototype._writeStylesToIDE = function() {
  ...
  FTIDE.prototype._showToastr = function (msg) {
    const toastr = new DOMElement().createElement('DIV').addClass('toastr').innerHTML(msg);
    this.ide.appendChild(toastr.node);
    setTimeout(() => {
      this.ide.removeChild(toastr.node);
    }, 2500);
  }
  FTIDE.prototype._setContent = function(content) {
