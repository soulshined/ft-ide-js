        ...
        for (const [key, value] of Object.entries(tree)) {
            ...
            if (typeof value === 'object' && value.constructor === Object) {
                ...
                if (child.build().firstChild)
                    ul.append(new ElementBuilder('li').append(child).addClass('subtree'));
            }
            else {
...
class IDE {
    ...
    writeToDOM(tree) {
        ...
        if (this.content.length > 0) {
            ...
            if (this.options.rootPath !== null) {
                ...
                if (matchedNode != null) {
                    ...
                    if (paths.length > 0 && matchedNode.classList.contains('folder')) {
                        ...
                        do {
                            canContinue = false;
                            if (matchedNode.nextSibling && matchedNode.nextSibling.tagName == 'LI' && matchedNode.nextSibling.classList.contains('subtree')) {
                                [...matchedNode.nextSibling.firstChild.children].filter(f => f.classList.contains('folder') || f.classList.contains('item')).forEach(child => {
            ...
            let padding = parseInt(target.style.paddingLeft);
            this.setContent(this.content[contentid]);
            this.updateBreadcrumbs(target);
            const owner = target.closest('div.explorer');
            if (owner !== null) owner.scrollTo(padding - 10, 0);
        }
    }
    getBranding() {
        return `<!--
...
-->`;
    ...
    updateBreadcrumbs(element) {
        ...
        if (this.options.showBreadcrumbs !== 'never') {
            let title = '';
            let lis = [];
            let node = element.closest('ul');
            while (!node.classList.contains('tree')) {
                const parent = node.closest('.subtree').previousSibling;
                lis.push(new ElementBuilder('li').innerText(parent.textContent));
                title = ` / ${parent.textContent}` + title;
                node = parent.closest('ul');
            }
            this.breadcrumbDiv.title = title + ` / ${element.textContent}`;
            ...
            this.breadcrumbDiv.innerHTML = `<li></li>` + lis.reverse().map(m => m.build().outerHTML).join('') + `<li>${element.textContent}</li>`;
        }
    }
    writeStylesToIDE() {
